...
static u64 uart_read(MemRegion *r, u64 offset, size_t width) {
  ...
  (void)width;
  switch (offset) {
  case UART_RBR: return (s_uart.lcr & UART_LCR_DLAB) ? s_uart.dll : 0;
  case UART_IER: return (s_uart.lcr & UART_LCR_DLAB) ? s_uart.dlh : s_uart.ier;
  case UART_IIR: return UART_IIR_NO_INT;
  case UART_LCR: return s_uart.lcr;
  case UART_MCR: return s_uart.mcr;
  case UART_LSR: return UART_LSR_TX_READY;
  case UART_MSR: return UART_MSR_READY;
  case UART_SCR: return s_uart.scr;
  default:       return 0;
  }
}
...
static void uart_write(MemRegion *r, u64 offset, u64 val, size_t width) {
  ...
  switch (offset) {
  case UART_THR:
    if (s_uart.lcr & UART_LCR_DLAB) { s_uart.dll = v; break; }
    putchar((int)v);
    fflush(stdout);
    break;
  case UART_IER:
    if (s_uart.lcr & UART_LCR_DLAB) { s_uart.dlh = v; break; }
    s_uart.ier = v;
    break;
  case UART_FCR: break; // FIFO control — absorb, no FIFO to manage
  case UART_LCR: s_uart.lcr = v; break;
  case UART_MCR: s_uart.mcr = v; break;
  case UART_SCR: s_uart.scr = v; break;
  default: break;
  }
}
